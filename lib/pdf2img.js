...
Pdf2Img.prototype.pageCompleted = function (callback) {pageCompletedArray.push(callback)}

Pdf2Img.prototype.fileCompleted = function (callback) {fileCompletedArray.push(callback)}

Pdf2Img.prototype.error = function (callback) {errorArray.push(callback)}

Pdf2Img.prototype.run = function (filename) {
 // Make sure it has the correct extension.
 if (path.extname(path.basename(input)) != '.pdf') {
  this.errorArray.forEach(function (entry) {
   entry('Unsupported file type.')
  })
  return
 }
 ...
 async.waterfall([
  ...
  function (pages, callback) {
   ...
   async.eachSeries(pages, function (page, callbackmap) {
    ...
    if (e) callback(e); return callback(null, {result: 'success', message: stdout})
   })
  }
 ], callbackreturn)
}
